...
public class Food_Truck_Account extends Exception {
	...
	public static double incomeAmount;
	private static double salesTaxCollected;
	
	//for the new core code
	double totalIncome = 0.0;
	double totalExpenses = 0.0;
	static double totalNYSalesTaxCollected  = 0.0;
	static double totalNJSalesTaxCollected = 0.0;
	static double totalCTSalesTaxCollected = 0.0;
	static double totalBostonSalesTaxCollected = 0.0;
	static double totalChicagoSalesTaxCollected = 0.0;
	//----------------end new core code varables
	...
	public static void main(String[] args) {
		...
		String fileName = "/Users/DUVALL NOTEBOOK/workspace/project 2018 version 8/src/data/food_truck.csv";
        Scanner inputStream;
		...
		try {
			...
			outputStream2 = new PrintWriter("/Users/DUVALL NOTEBOOK/workspace/project 2018 version 8/src/public_html_Food_Truck/Food Truck Company Statement"+ numCounter +".html");
			outputStream3 = new PrintWriter("/Users/DUVALL NOTEBOOK/workspace/project 2018 version 8/src/public_html_Food_Truck/Food Truck Company Statement"+ numCounter +".css");
			...
			outputStream2.println("<nav> <a href='mainpage.html'>Main page</a> <a href='Food_Truck Home.html'>Food Truck Home page</a></nav>");
            ...
            outputStream3.println("}");
				...
				/*
				// begin code for monthly summary
				// split data string by "month/day/year"
				// values[2].split("/")
				int month = 0;
				int day = 0;
				int year = 0000;
				String dateString[] = values[2].split("/");
				month = Integer.parseInt(dateString[0]);
				day = Integer.parseInt(dateString[1]);
				year = Integer.parseInt(dateString[2]);
				
				outputStream2.println("<p>Transaction for the date"+month+"/"+day+"/"+year+"</p>");
				
				
				if (month == 1)
				{
					//values[3]= income, values[4]= expenses, values[5]= sales tax
					//keep counters for sales tax, expenses , and income for the month
				    // totalNYSalesTaxCollected;
					//   totalNJSalesTaxCollected;
					// totalCTSalesTaxCollected;
					// totalBostonSalesTaxCollected;
					// totalChicagoSalesTaxCollected;
					
					incomeAmount = Double.parseDouble(income);
					expenses = Double.parseDouble(values[4]);
					if (state == "NY")
					{
						salesTaxCollected = Double.parseDouble(values[5]);
						totalNYSalesTaxCollected += salesTaxCollected;
						outputStream2.println("<p>");
					}
					else if(state == "NJ")
					{
						salesTaxCollected = Double.parseDouble(values[5]);
						totalNJSalesTaxCollected += salesTaxCollected;
					}
					else if(state == "CT")
					{
						salesTaxCollected = Double.parseDouble(values[5]);
						totalCTSalesTaxCollected += salesTaxCollected;
					}
					else if (state == "Boston")
					{
						salesTaxCollected = Double.parseDouble(values[5]);
						totalBostonSalesTaxCollected += salesTaxCollected;
					}
					else if (state == "Chicago")
					{
						salesTaxCollected = Double.parseDouble(values[5]);
						totalChicagoSalesTaxCollected += salesTaxCollected;
					}
	...
	// ************************* //
		...
		public static double displayAccountBalance(double accountBalance){
			accountBalance = Food_Truck_Account.accountBalance;
			return accountBalance;
		}
		...
		public static double displayEscoBalance(double escoBalance){
			escoBalance = Food_Truck_Account.escoBalance;
			return escoBalance;
		}
